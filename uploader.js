 ...
 * http://uploaderjs.com/
 *
 * Copyright 2013, Pap Tamas
 * Released under the MIT license
 * http://www.opensource.org/licenses/MIT
 *
 * Hey Developer! It's nice to see you here. I hope you'll enjoy checking out the source code of UploaderJS.
 * I tried to make it as clean, organized and well documented as possible. However, if you have any questions, ideas or
 * tips for improving it, visit the support page or drop me a line at paptamas [at] gmail.com.
 *
 * UploaderJS was tested successfully in all major browsers (IE6+, Firefox 3.0+, Safari 4.0+, Chrome, Opera 10.4+), however,
 * if you discover a bug, please report it on the project's github page (https://github.com/paptamas/uploader-js/issues).
 * 
 * Ready? Let's start!
 */
 ...
 // Create the Uploader
window.Uploader = (function ($, window) {    
    ...
    $.extend(Uploader, {
        ...
        defaults: {
            ...
            // The button to launch the file browser dialog (possible values: a jQuery selector/object or an HTMLElement)
            selectButton: null,

            // The drop zone where the user can drop files (possible values: a jQuery selector/object or an HTMLElement)
            dropZone: null,
            ...
            // Whether to automatically upload files after they were selected/dropped
            autoUpload: true,
...
// Extend prototype: Implement initialization
window.Uploader = (function (Uploader, $) {
    ...
    $.extend(Uploader.prototype, {
        ...
        init: function(options) {
            ...
            // Merge options with the default values
            this.options = $.extend(true, {}, this.constructor.defaults, options);

            // Because on a jquery deep extend arrays are extended too, we have to set `acceptType` manually
            this.options.acceptType = options.acceptType || this.constructor.defaults.acceptType;
            ...
            /**
             * All the files selected/dropped by the user are added one by one to the file list (if they pass the validation)
             *
         ...
         *
         * Basically we create a container div and place a transparent file input in it. Then we position the container over the
         * select button. So users will see the select button, but will actually click the transparent file input, thus
         * the file browser dialog will be opened.
         */
        initElements: function() {

            // Init the select button
            this.selectButtonElement = $(this.options.selectButton);

            // Create the container
            this.containerElement = $('<div/>').css({
                width: this.selectButtonElement.outerWidth(),
                height: this.selectButtonElement.outerHeight(),
                cursor: this.selectButtonElement.css('cursor'),
                overflow: 'hidden',
                border: 'none',
                background: '#ffffff',
                opacity: 0
            ...
            this.containerElement.appendTo(document.body);

            /**
             * Add and remove css classes to the select button on container's mouse enter, mouse leave and mouse down
             */
            this.containerElement.on('mouseenter', $.proxy(function() {
                this.selectButtonElement.addClass(this.options.cssClasses.selectButtonHover);
            }, this));

            this.containerElement.on('mouseleave', $.proxy(function() {
                this.selectButtonElement
                    .removeClass(this.options.cssClasses.selectButtonHover)
                    .removeClass(this.options.cssClasses.selectButtonFocus);
            }, this));

            this.containerElement.on('mousedown', $.proxy(function() {
                this.selectButtonElement.addClass(this.options.cssClasses.selectButtonFocus);
            }, this));

            this.containerElement.on('mouseup', $.proxy(function() {
                this.selectButtonElement.removeClass(this.options.cssClasses.selectButtonFocus);
            }, this));

            // Create the transparent file input and append it to the container
         ...
         * Position the container over the select button
         *
         * You can call this method any time when - for some reason - the position of the select button changes, so the
         * container's position must be also updated.
         */
        ...
        createFileInput: function() {
            ...
            if (this.options.multiple && this.constructor.support.selectMultiple && this.isModernBrowser) {
                this.fileInputElement.attr('multiple', 'multiple');
            }

            // Move the transparent file input with the cursor in the container, to make sure it gets clicked
            this.containerElement.off('mousemove').on('mousemove', '', $.proxy(function(e) {
                this.fileInputElement.css({
                    top: e.pageY - this.containerElement.offset().top - 10 + 'px',
                    left: e.pageX - this.containerElement.offset().left - this.fileInputElement.outerWidth() + 10 + 'px'
                });
            }, this));

            // Listen to file input's `onchange` event
            this.fileInputElement.on('change', '', $.proxy(this.onFileSelect, this));
        },
        ...
        initDropZone: function() {
            this.dropZoneElement = $(this.options.dropZone);

            // On drag over
            this.dropZoneElement.on('dragover.Uploader', $.proxy(function(e) {
                e.preventDefault();
                this.dropZoneElement.addClass(this.options.cssClasses.dropZoneDragOver);

                return false;
            }, this));

            // On drag end
            this.dropZoneElement.on('dragend.Uploader', $.proxy(function(e) {
                e.preventDefault();
                this.dropZoneElement.removeClass(this.options.cssClasses.dropZoneDragOver);

                return false;
            }, this));

            // On drag leave
            this.dropZoneElement.on('dragleave.Uploader', $.proxy(function(e) {
                e.preventDefault();
                this.dropZoneElement.removeClass(this.options.cssClasses.dropZoneDragOver);
            ...
            // On drop
            this.dropZoneElement.on('drop.Uploader', $.proxy(function(e) {
                e.preventDefault();
                this.dropZoneElement.removeClass(this.options.cssClasses.dropZoneDragOver);
...
window.Uploader = (function (Uploader, $) {
    ...
    $.extend(Uploader.prototype, {
        ...
        onFileSelect: function() {
            ...
            var files = this.isModernBrowser ? this.fileInputElement[0].files : [this.fileInputElement];
            ...
            if ( ! this.isModernBrowser) {
                ...
                this.fileInputElement.off().detach();
            }
            else {
        ...
        onUploadComplete: function(uniqueId, name, response, status, xhr) {
             ...
             * We do this to make Uploader totally independent from the server side implementation. So it can be used
             * even when developers have no control over how and in what format the server responds to requests, but based
             * on server's response they can decide if the upload was actually successful or not (this is the case when you upload
             * files to Amazon S3 for example.).
             *
             * Therefore when an upload is completed, it's the developers job to check if the upload was successful or not,
             * based on the server's response. If the upload was unsuccessful, developers will throw an error with the
 ...
 * 2.   After they are validated, the file objects from the file input or the drop zone are added to
 *      the file list
 *
 * Uploading a file:
 *
...
window.Uploader = (function (Uploader, $, window, undefined) {
    ...
    $.extend(Uploader.prototype, {
         ...
         *
         * This method is called when new files are selected or dropped to the drop zone.
         *
         * @param files
         */
            ...
            $.each(files, $.proxy(function(i, file) {
                ...
                if ( ! $.isNumeric(this.options.maxFiles) || this.fileCount < this.options.maxFiles) {
                    try {
                        ...
                        this.fileInputElement.attr('id', uniqueId);
        ...
        validateFile: function(file) {
            ...
            // We will collect validation errors in this array
            var errors = [];
        ...
        getUploadedBytes: function(uniqueId) {
            ...
            if (uniqueId) {
                return this.isUploading(uniqueId)
                    ? this.fileList[uniqueId].progress.current.bytes
                    : 0;
            }
            else {
                var total = 0;
        ...
        getUploadSpeed: function(uniqueId, average) {
            ...
            (average === undefined) && (average = true);

            if (uniqueId) {
                var progress = this.fileList[uniqueId].progress;
                return (progress.current.bytes && progress.current.bytes != this.getBytes(uniqueId))
                    ? average
                        ? Math.round(progress.current.bytes / ((progress.current.time - progress.startTime) / 1000))
                        : Math.round((progress.current.bytes - progress.previous.bytes) / ((progress.current.time - progress.previous.time) / 1000))
                    : 0;
            }
            else {
                var total = 0;
        ...
        destroy: function() {
            ...
            this.dropZoneElement && this.dropZoneElement.off('.Uploader');
        ...
        createIFrame: function(uniqueId) {
            ...
            iFrame.off().on('load', $.proxy(function() {
                ...
                this.readIframe(uniqueId, iFrame, 0);
            }, this));
         ...
         *
         * In some browsers we have to wait a little bit after the iFrame's `onload` event fires, to be able to read it's
         * content (bug in Opera 10.0 (and maybe some other versions))
         *
         * @param uniqueId
         * @param iFrame
         * @param n
         */
        readIframe: function(uniqueId, iFrame, n) {
            ...
            if (response || n >= 5) {
                ...
                setTimeout($.proxy(function() {
                    this.readIframe(uniqueId, iFrame, n + 1);
                }, this), 1000);
            }
        },
